...
import {
  generateDigestIfDue,
  processSourceItem,
  queueLatestWeeklyTranslationRepair,
...
function parseBrief<T extends { highlights_json: string }>(row: T | null) {
  ...
  try {
    highlights = JSON.parse(highlights_json) as unknown[];
  } catch {
    highlights = [];
  }
...
async function dashboard(env: Env) {
  const [stats, categories, knowledge, news, daily, weekly] = await Promise.all(
    ...
    `,
      ...
      env.DB.prepare(
        `${ARTICLE_SELECT} WHERE a.status = 'published' AND a.content_type = 'knowledge' ORDER BY datetime(a.updated_at) DESC LIMIT 8`,
      ).all<ArticleRow>(),
      env.DB.prepare(
        `${ARTICLE_SELECT} WHERE a.status = 'published' AND a.content_type = 'news' ORDER BY datetime(a.published_at) DESC LIMIT 8`,
      ).all<ArticleRow>(),
      env.DB.prepare(
        "SELECT id, brief_type, title, intro, highlights_json, period_start, period_end, published_at FROM briefs WHERE status = 'published' AND brief_type = 'daily' ORDER BY datetime(published_at) DESC LIMIT 1",
      ).first<{
        id: number;
        brief_type: "daily";
        title: string;
      ...
      env.DB.prepare(
        "SELECT id, brief_type, title, intro, highlights_json, period_start, period_end, published_at FROM briefs WHERE status = 'published' AND brief_type = 'weekly' ORDER BY datetime(published_at) DESC LIMIT 1",
      ).first<{
        id: number;
        brief_type: "weekly";
        title: string;
...
async function getArticle(slug: string, env: Env) {
  const row = await env.DB.prepare(
    `${ARTICLE_SELECT} WHERE a.slug = ? AND a.status = 'published' LIMIT 1`,
  )
    .bind(slug)
    .first<ArticleRow>();
  return row
...
async function ask(request: Request, env: Env) {
  ...
  const result = await env.DB.prepare(
    `${ARTICLE_SELECT} WHERE a.status = 'published' ORDER BY datetime(a.updated_at) DESC LIMIT 100`,
  ).all<ArticleRow>();
  ...
  const ranked = (result.results ?? [])
    .map((row) => {
      ...
      const bodyGrams = ngrams(
        `${article.summary}${article.tags.join("")}${article.content.slice(0, 2500)}`,
      );
      let score = 0;
      questionGrams.forEach((gram) => {
        if (titleGrams.has(gram)) score += 4;
        if (bodyGrams.has(gram)) score += 1;
      });
  ...
  const contextSources = sources.length
    ? sources
    : ranked.slice(0, 3).map((item) => item.article);
...
async function listSources(request: Request, env: Env) {
  ...
  const result = await env.DB.prepare(
    "SELECT id, name, feed_url, site_url, source_type, trust_level, active, last_checked_at, last_success_at, last_error FROM sources ORDER BY trust_level DESC, name ASC",
  ).all();
...
async function addSource(request: Request, env: Env) {
  ...
  if (!name || !isSafeFeedUrl(feedUrl))
    return error("请填写有效的来源名称和公开Feed地址。", 422);
  if (siteUrl && !isSafeFeedUrl(siteUrl))
    return error("官网地址格式无效。", 422);
  ...
  `,
    ...
    .bind(name, feedUrl, siteUrl || new URL(feedUrl).origin, trust)
    .run();
  const source = await env.DB.prepare(
    "SELECT id, name, feed_url, site_url, source_type, trust_level, active, last_checked_at, last_success_at, last_error FROM sources WHERE id = ?",
  )
...
async function repairWeeklyChinese(request: Request, env: Env) {
  if (!isAuthorized(request, env.ADMIN_TOKEN))
    return error("管理员令牌无效。", 401);
  const queued = await queueLatestWeeklyTranslationRepair(env);
  return json({ queued });
}
...
async function handleFetch(request: Request, env: Env) {
  ...
  if (request.method === "GET" && path.startsWith("/api/articles/"))
    return getArticle(
      decodeURIComponent(path.slice("/api/articles/".length)),
      env,
    );
  if (request.method === "GET" && path === "/api/digests/latest") {
    const type = url.searchParams.get("type") === "weekly" ? "weekly" : "daily";
    const row = await env.DB.prepare(
      "SELECT id, brief_type, title, intro, highlights_json, period_start, period_end, published_at FROM briefs WHERE status = 'published' AND brief_type = ? ORDER BY datetime(published_at) DESC LIMIT 1",
    )
      .bind(type)
      .first<{
        id: number;
  ...
  if (request.method === "GET" && path === "/api/admin/sources")
    return listSources(request, env);
  if (request.method === "POST" && path === "/api/admin/sources")
    return addSource(request, env);
  if (request.method === "POST" && path === "/api/admin/scan")
    return manualScan(request, env);
  if (request.method === "POST" && path === "/api/admin/retranslate")
  ...
  if (request.method === "POST" && path.startsWith("/api/admin/digests/")) {
    if (!isAuthorized(request, env.ADMIN_TOKEN))
      return error("管理员令牌无效。", 401);
    const type = path.endsWith("/weekly")
      ? "weekly"
      : path.endsWith("/daily")
        ? "daily"
        : null;
...
export default {
  ...
  async scheduled(
    ...
    if (controller.cron === "0 */6 * * *") {
      ctx.waitUntil(
        Promise.all([scanAllSources(env), queueUntranslatedNews(env, 100)]),
      );
    } else if (controller.cron === "30 0 * * *")
      ctx.waitUntil(generateDigestIfDue(env, "daily"));
    else if (controller.cron === "0 1 * * SUN")
      ctx.waitUntil(generateDigestIfDue(env, "weekly"));
  },

  async queue(batch: MessageBatch<QueueJob>, env: Env) {
    for (const message of batch.messages) {
      try {
        if (message.body.type === "scan_source")
          await scanSource(env, message.body.sourceId);
        if (message.body.type === "process_item")
          await processSourceItem(env, message.body.sourceItemId);
        if (message.body.type === "reprocess_item")
          await processSourceItem(env, message.body.sourceItemId, true);
        message.ack();
      } catch (cause) {
