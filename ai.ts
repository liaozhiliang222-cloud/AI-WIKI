...
async function translateWithChatModel(
      ...
      messages: [
        {
          role: "system",
          content:
            "你是专业中文科技编辑。只做忠实翻译与编辑，不添加原文没有的事实。输出严格JSON，不要使用Markdown。",
        },
        {
          role: "user",
          content: `请把下面英文AI资讯翻译并整理为适合中文读者阅读的内容。输出字段 translated_title、translated_content。translated_title必须是自然中文标题；translated_content控制在600-1200个中文字符，保留关键事实、数字、产品名、限制与适用对象。\n\n标题：${title}\n正文：${clampText(content, 9000)}`,
        },
      ],
      max_tokens: 2400,
      temperature: 0.1,
  ...
  return {
    title: clampText(String(parsed.translated_title || ""), 220),
    content: clampText(String(parsed.translated_content || ""), 7000),
  };
}
...
export async function summarizeArticle(
  ...
  try {
    ...
    const parsed = parseJsonFromModel(modelText(result));
    const resolvedLanguage = ["zh", "en", "other"].includes(
    ...
    if (
      resolvedLanguage === "en" &&
      (chineseRatio(translatedTitle) < 0.08 ||
        chineseRatio(translatedContent) < 0.08)
    ) {
      try {
        ...
        if (chineseRatio(chatTranslation.title) >= 0.08)
          translatedTitle = chatTranslation.title;
        if (chineseRatio(chatTranslation.content) >= 0.08)
          translatedContent = chatTranslation.content;
      } catch (translationError) {
        console.warn("Chat translation fallback failed", translationError);
      }
    }
    ...
    if (
      ...
      try {
        ...
        if (chineseRatio(titleTranslation) >= 0.08)
          translatedTitle = clampText(titleTranslation, 220);
        if (chineseRatio(contentTranslation) >= 0.08)
          translatedContent = clampText(contentTranslation, 6000);
      } catch (translationError) {
    ...
    if (sourceLanguage === "en") {
      try {
        ...
        if (
          chineseRatio(chatTranslation.title) >= 0.08 ||
          chineseRatio(chatTranslation.content) >= 0.08
        ) {
          return {
            ...fallback,
            translatedTitle: chatTranslation.title || fallback.translatedTitle,
            translatedContent:
              chatTranslation.content || fallback.translatedContent,
            summary: chatTranslation.content
              ? clampText(chatTranslation.content, 220)
              : fallback.summary,
          };
        }
      } catch (translationError) {
        console.warn("Chat translation-only fallback failed", translationError);
      }
      try {
